...
def cut_qts_to_words(qts_file,saved_words_file):
    ...
    dumped_file=os.path.join(save_dir,'qts_words_start_result.pkl')
...
def print_stat_results(char_counter, author_counter, genre_counter,vector_model):
    ...
    # 地名排名
    print('\n地名词排名')
    print_counter(genre_counter['ns'].most_common(12))
    # 时间排名
    print('\n时间词排名')
    ...
    print_counter(genre_counter['n'].most_common(10))
    ...
    # print_similar_words('快乐',vector_model)
    # print_similar_words('愤怒',vector_model)
    # print(vector_model.wv.similarity('愤怒', '生气'))

def tangmainjieba(qts_path,words_path):

    # parser=argparse.ArgumentParser()
    # parser.add_argument('--qts_path', type=str, default='../data/json_zh/poet_tang.json',
    #                   help='file path of Quan Tangshi')
    # #储存分词后的诗词word
    # parser.add_argument('--words_path', type=str, default='../save_Skip1/tang_words_list.txt',
    #                   help='file path to save_Skip1 Quan Tangshi words data')
    ...
    return char_counter,author_counter,genre_counter,vector_model

if __name__ == '__main__':
    qtspath='../data/json_zh/poet_tang.json'
    wordspath='../save_Skip2/tang_words_list.txt'
    tangmainjieba(qtspath,wordspath)
